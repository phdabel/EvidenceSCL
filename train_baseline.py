...
import pickle
import warnings
from util import save_model, parse_option, get_dataloaders, compute_nli4ct_nli_metric, epoch_summary
...
def main_worker(gpu, args):
    ...
    for epoch in range(args.epochs):
        ...
        if args.dataset == 'nli4ct':
            train_semeval_maj_metric, \
                train_semeval_at_least_one_metric, \
                train_agg_results = compute_nli4ct_nli_metric(train_result, args, 'training')

            train_semeval_metric = max(train_semeval_maj_metric, train_semeval_at_least_one_metric)
            train_semeval_metric = torch.tensor([train_semeval_metric], dtype=torch.float32)

            val_semeval_maj_metric, \
                val_semeval_at_least_one_metric, \
                val_agg_results = compute_nli4ct_nli_metric(val_result, args, 'validation')

            val_semeval_metric = max(val_semeval_maj_metric, val_semeval_at_least_one_metric)
