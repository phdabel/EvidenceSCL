...
for dataset in "nli4ct" "mednli" "multinli"
do
  ...
  do
    ...
    do
      ...
      do
        ...
        python test_baseline.py --dataset "$dataset" --max_seq_length "$max_seq_length" --num_classes "$num_classes" --model_name biomed --batch_size "$BATCH_SIZE" --gradient_accumulation_steps "$GRADIENT_ACCUMULATION_STEPS" --epochs "$EPOCHS"  --workers "$WORKERS" --learning_rate "$learning_rate"
      done
    done
  done
done
