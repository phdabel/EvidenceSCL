...
def get_dataset_from_dataframe_2(data, tokenizer, classdict=None, max_length=128, num_labels=2, semeval_only=False):
    ...
    if num_labels == 2:
        all_class_labels = torch.tensor([classdict[sample.two_labeled_class] for i, sample in data.iterrows()],
                                        dtype=torch.long)
    else:
        all_class_labels = torch.tensor([classdict[sample.three_labeled_class] for i, sample in data.iterrows()],
                                        dtype=torch.long)
