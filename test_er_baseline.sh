...
do
  ...
  do
    ...
    python test_er_baseline.py --dataset nli4ct --max_seq_length "$max_seq_length" --num_classes 2 --model_name biomed --batch_size "$BATCH_SIZE" --gradient_accumulation_steps "$GRADIENT_ACCUMULATION_STEPS" --epochs "$EPOCHS"  --workers "$WORKERS" --learning_rate "$learning_rate"
  done
done